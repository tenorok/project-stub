({
    block: 'page',
    title: 'Контакты',
    favicon: '/favicon.ico',
    head: [
        { elem: 'css', url: '_index.css', ie: false }
    ],
    content: [
        {
            block: 'contacts',
            content: [
                {
                    elem: 'list',
                    content: [
                        {
                            block: 'contact',
                            photo: 'http://tenorok.github.io/shri-bem-practice/store/homer.png',
                            group: 'семья',
                            name: 'Гомер Джей Симпсон',
                            phone: '+7 987 456-71-28',
                            address: 'Спрингфилд, 36',
                            email: 'homer@yandex.ru',
                            details: [
                                { 'День рождения': '1 января 1970' },
                                { 'Место работы': 'АЭС' },
                                { 'Отдел': 'Департамент утечек атомного реагента' },
                                { 'Должность': 'Специалист по безопасности' }
                            ],
                            social: {
                                facebook: 'https://www.facebook.com/pages/Homer-Simpson/109386305754801',
                                twitter: 'https://twitter.com/HomerJSimpson',
                                vkontakte: 'http://vk.com/id117179044'
                            }
                        },
                        {
                            block: 'contact',
                            photo: 'http://tenorok.github.io/shri-bem-practice/store/willy.png',
                            group: 'школа',
                            name: 'Вилли',
                            phone: '+7 126 942-33-47',
                            address: 'Улица Джордано, 1',
                            email: 'willey@yandex.ru',
                            details: [
                                { 'День рождения': '20 марта 1990' },
                                { 'Место работы': 'Школа' },
                                { 'Должность': 'Садовник' }
                            ],
                            social: {
                                vkontakte: 'http://vk.com/id221846237'
                            }
                        },
                        {
                            block: 'contact',
                            photo: 'http://tenorok.github.io/shri-bem-practice/store/apu.png',
                            group: 'друзья',
                            name: 'Апу Нахасапимапетилон',
                            phone: '+7 683 485-67-20',
                            address: 'Спрингфилд, 18',
                            email: 'apu@yandex.ru',
                            details: [
                                { 'Место работы': 'Магазин Kwik-E-Mart' },
                                { 'Должность': 'Владелец' }
                            ],
                            social: {
                                facebook: 'https://www.facebook.com/pages/Apu-Apouh-the-simpsons/41486825419',
                                vkontakte: 'http://vk.com/id224379274'
                            }
                        },
                        {
                            block: 'contact',
                            photo: 'http://tenorok.github.io/shri-bem-practice/store/mo.png',
                            group: 'друзья',
                            name: 'Моррис Лестер Сизлак',
                            phone: '+7 943 344-55-74',
                            address: 'Спрингфилд, 11',
                            email: 'moe@yandex.ru',
                            details: [
                                { 'День рождения': '22 февраля 1966' },
                                { 'Место работы': 'Бар «У Мо»' },
                                { 'Должность': 'Владелец и бармен' }
                            ],
                            social: {
                                facebook: 'https://www.facebook.com/Moeszyslaktavern?hc_location=stream',
                                twitter: 'https://twitter.com/PiadasMoe_'
                            }
                        },
                        {
                            block: 'contact',
                            photo: 'http://tenorok.github.io/shri-bem-practice/store/otto.png',
                            group: 'школа',
                            name: 'Отто Манн',
                            phone: '+7 185 926-23-06',
                            address: 'Шестая авеню, 67',
                            email: 'otto@yandex.ru',
                            details: [
                                { 'День рождения': '16 июня 1990' },
                                { 'Место работы': 'Школа' },
                                { 'Должность': 'Водитель автобуса' }
                            ],
                            social: {
                                facebook: 'https://www.facebook.com/pages/otto-mann/199644113399563',
                                twitter: 'https://twitter.com/OttoBusdriver',
                                vkontakte: 'https://twitter.com/OttoBusdriver'
                            }
                        },
                        {
                            block: 'contact',
                            customize: 'vip',
                            photo: 'http://tenorok.github.io/shri-bem-practice/store/monty.png',
                            group: 'работа',
                            name: 'Чарльз Монтгомери Бёрнс',
                            phone: '+7 999 666-66-66',
                            address: 'Спрингфилд, 66',
                            email: 'burns@yandex.ru',
                            details: [
                                { 'Место работы': 'Спрингфилдская атомная электростанция' },
                                { 'Отдел': 'Правление' },
                                { 'Должность': 'Генеральный директор' }
                            ],
                            social: {
                                vkontakte: 'http://vk.com/id165968709'
                            }
                        },
                        {
                            block: 'contact',
                            photo: 'http://tenorok.github.io/shri-bem-practice/store/skinner.png',
                            group: 'школа',
                            name: 'Сеймур Скиннер',
                            phone: '+7 934 432-22-45',
                            address: 'Кэпитал-сити, 28',
                            email: 'skinner@yandex.ru',
                            details: [
                                { 'Дата рождения': '12 апреля 1953' },
                                { 'Место работы': 'Спрингфилдская начальная школа' },
                                { 'Должность': 'Директор' }
                            ],
                            social: {
                                facebook: 'https://www.facebook.com/pages/Seymour-Skinner/108122642541112?rf=132585913446468',
                                twitter: 'https://twitter.com/Seymour_Skinner'
                            }
                        },
                        {
                            block: 'contact',
                            customize: 'vip',
                            photo: 'http://tenorok.github.io/shri-bem-practice/store/quimby.png',
                            name: 'Джозеф Фицпатрик Фицджеральд Фицэнри Квимби младший',
                            phone: '+7 232 232-22-22',
                            email: 'quimby@yandex.ru',
                            details: [
                                { 'Дата рождения': '18 сентября 1957' },
                                { 'Место работы': 'Городская мэрия' },
                                { 'Должность': 'Мэр' }
                            ]
                        },
                        {
                            block: 'contact',
                            photo: 'http://tenorok.github.io/shri-bem-practice/store/burny.png',
                            name: 'Барнард Гамбл',
                            phone: '+7 954 332-36-18',
                            details: [
                                { 'Место работы': 'Безработный' },
                                { 'Хобби': 'Алкоголик' }
                            ]
                        }
                    ]
                },
                {
                    block: 'add-contact',
                    content: 'Добавить контакт'
                }
            ]
        },
        { elem: 'js', url: '_index.js' }
    ]
})
